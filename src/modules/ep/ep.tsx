...
export default function EPsPage() {
  ...
  const [editingEP, setEditingEP] = useState<EP | null>(null);
  ...
  const [epData, setEPData] = useState(eps);
  const router = useRouter();

  // Fetch artistes et tags
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchCovers = async () => {
      ...
      const updatedEPs = await Promise.all(
        eps.map(async (ep) => {
          const coverImageUrl = await getImageFile(ep.coverUrl!, token!);
          return { ...ep, coverImageUrl };
        })
      );
      setEPData(updatedEPs);
    };
    fetchCovers();
  }, [eps]);
  ...
  async function fetchEPs() {
    ...
    try {
      if (!token) return;
      const res = await ModuleObject.service.getEpByUser(userId!, token);
      setEPs(res.eps);
      setSuccess(true);
    } catch (error) {
      setErrorMessage((error as Error).message);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.title) newErrors.title = "Le titre de l’EP est requis.";
    ...
    try {
      if (editingEP) {
        await ModuleObject.service.updateEp(editingEP.id!, {
          title: form.title,
        ...
        const newEP = await ModuleObject.service.createEp(payload, token);
        if (form.tagId) {
          await ModuleObject.service.createEpTag(form.tagId, newEP.data.id);
        }
      }
      resetForm();
      fetchEPs();
    } catch (err) {
      setErrorMessage("Erreur lors de la sauvegarde de l’EP");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function handleDelete(id: string) {
    if (!confirm("Voulez-vous vraiment supprimer cette EP ?")) return;
    setLoading(true);
    setError(null);
    try {
      await ModuleObject.service.deleteEp(id);
      fetchEPs();
    } catch {
        ...
        <div className="min-h-screen bg-gray-100 p-6 w-full max-w-4xl">
          ...
          >
            <h2 className="text-xl font-semibold mb-4 text-[#1A4C61]">
              {editingEP ? "Modifier une EP" : "Ajouter une EP"}
            </h2>
            ...
            <div className="flex gap-4 justify-end">
              ...
              >
                {editingEP ? "Mettre à jour" : "Ajouter"}
              </button>
              {editingEP && (
                <button
                  type="button"
          ...
          <div className="bg-white rounded shadow p-4">
            ...
            {!loading && eps.length === 0 && <p>Aucune EP disponible.</p>}
                ...
                >
                  <div>
                    ...
                    <p className="font-semibold">
                      ...
                      <span className="text-[#1F89A5]">{ep.user?.name}</span>
                    </p>
                    <p className="font-semibold">
                      <span className="text-[#1A4C61]">Titre : </span>
                      <span className="text-[#1F89A5]">{ep.title}</span>
                    </p>
                    <p className="font-semibold">
